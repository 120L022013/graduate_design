...
save_path = './saved_dict/lstm_sen.pth'        # 模型训练结果
# 文件路径
...
def train( model, dataloaders):
    ...
    class_weights = torch.tensor([1.0, 8.0], device='cuda:0')
    ...
    plot_F1('LSTM_sent',plot_train_F1,plot_dev_F1)
    plot_loss('LSTM_sent',plot_train_loss,[])
...
def dev_eval(model, data, loss_function,Result_test=True):
    ...
    f= 2*acc*rec/(acc+rec)
    if Result_test:
        result_test('LSTM_sent',labels_all, predict_all)
    else:
        pass
    print(acc, rec, f)
...
if __name__ == '__main__':
    ...
    loss_function = torch.nn.CrossEntropyLoss(weight=class_weights)
    model.load_state_dict(torch.load(save_path))
    test_acc, test_rec, test_f, test_loss = dev_eval(model,dataloaders['test'],loss_function)
