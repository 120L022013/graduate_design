...
num_epochs = 16
# 定义损失函数（使用类别权重）
class_weights = torch.tensor([1.0, 9.0], device='cuda:0')  # 根据数据分布设置类别权重
criterion = nn.CrossEntropyLoss(weight=class_weights)

class Mental_roberta(nn.Module):
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            if val_f1_score > max_f1 :
                max_f1 = val_f1_score
                torch.save(self.model.state_dict(), "./saved_dict/mental_roberta.pth")  # Save the best model weights
        plot_F1('mental_roberta', train_f1, dev_f1)
        plot_loss('mental_roberta', train_loss, [])
        # Load the best model weights
        self.model.load_state_dict(torch.load("./saved_dict/mental_roberta.pth"))
    ...
    def evaluate(self, eval_data):
        ...
        print(all_labels,'\n',all_preds)
        result_test('Mental_ROBERTa', all_labels, all_preds)
...
if __name__ == '__main__':
    ...
    # 初始化模型
    sentiment_model = Mental_roberta(
        model_name=MODEL,
    ...
    sentiment_model.load_state_dict(torch.load('./saved_dict/mental_roberta.pth'))
    print("finished")
